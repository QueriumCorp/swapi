...
const cleanAllTags = (str) => {
  let rslt = str.replace(
    /(\[ERRTYPE:START\](.*?)\[ERRTYPE:END\])|(\[HINTTYPE:START\](.*?)\[HINTTYPE:END\])|(\[FBTTYPE:START\](.*?)\[FBTTYPE:END\])/g,
    ""
  );
  rslt = rslt.replace(
    /\[ERRMSG:START\]|\[ERRMSG:END\]|\[HINTMSG:START\]|\[HINTMSG:END\]|\[FBMSG:START\]|\[FBMSG:END\]/g,
    ""
  );
...
export { createQueryString, parseResponse, handleFetch, containsAnyTagsQ };
